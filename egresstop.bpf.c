...
/*
 * egresstop — outbound TCP connection watchdog.
 *
 * Every TCP connection the box opens is observed by the kernel, paired
 * with the calling process, and streamed to userspace. The userspace
 * side then matches each connection's destination against an
 * allowlist: anything that doesn't match is flagged as a potential
 * exfiltration or C2 callback. Process attribution survives the
 * softirq problem because tcp_sendmsg / tcp_cleanup_rbuf always run
 * in app context with the real `current`.
 *
 * Three CO-RE hooks — proven safe surface used by every other tool in
 * this family. The two fentry hooks are on functions stable since the
 * 2.6 era (tcp_sendmsg) and only slightly newer (tcp_cleanup_rbuf
 * predates eBPF itself), so they meet the "untouched for decades"
 * bar for fentry stability.
 *
 ...
 *
 * One HASH map (conns, keyed by sock pointer) holds per-conn state.
 * One RINGBUF (256 KiB) carries three event kinds to JS. All
 * allowlist matching, alert generation, and policy lives in
 * userspace — kernel side knows nothing about the rules.
 */
...
/* Refuse kernel-thread names as the "real owner". TCP state transitions
 * can fire from softirq, where current is whatever happens to be
 * scheduled. tcp_sendmsg / tcp_cleanup_rbuf in app context will give
 * us the real owner. */
